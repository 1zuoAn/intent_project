...
REAL_LLM_MODEL = os.getenv("REAL_LLM_MODEL", "google/gemini-3-flash-preview")
...
ES_URL = f"http://{ES_USER}:{ES_PASS}@{ES_HOST}" if ES_USER else None
...
1. **选品**
   ...
   - **核心排他**: 如果用户目的是找“同款”或“相似款”，请归类为【图搜】，而非选品。

2. **图搜**
   - **定义**: 用户明确意图寻找“同款”、“相似款”，或者基于图片/特定款式进行视觉搜索。
   - **触发时机**: 涉及“同款”、“相似”、“搜图”、“这张图”、“基于这个款式”等字眼，或明确要求“找图片上的款”。

3. **生图改图**
   - **定义**: 用户明确意图生成图片（如设计服装）或修改现有图片（如换背景、模特）。涉及图像创建或编辑。
   - **触发时机**: 涉及生成、设计字眼时触发。
...
5. **媒体**
   - **定义**: 用户明确意图搜索 Instagram (ins) 或 小红书 上的帖子内容、博主动态。
